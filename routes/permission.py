...
def view(id):
    ...
    if user_role == 'manager':
        ...
        # Admins can approve if status is pending, manager and director have approved
        ...
        if approval_form.status.data == 'approved':
            if user_role == 'manager':
                ...
                for admin in get_user_managers(requester)['admin_managers']:
                    create_notification(
                        user_id=admin.id,
                        message=f"Permission request from {requester.get_full_name()} approved by director and needs your review",
                        notification_type='approval',
                        reference_id=permission_request.id,
                        reference_type='permission'
                    )
